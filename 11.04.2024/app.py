...
def get_bitcoin_data():
    try:
        # Указываем URL API для получения данных о биткоине
        url = 'https://api.coingecko.com/api/v3/coins/bitcoin'
        response = requests.get(url)
        if response.status_code == 200:
            data = response.json()
            # Обработка и извлечение необходимых данных из ответа
            processed_data = {
                'open': data['market_data']['current_price']['usd'],  # Примерная логика, нужно адаптировать под ваши нужды
                'high': data['market_data']['high_24h']['usd'],
                'low': data['market_data']['low_24h']['usd'],
                'close': data['market_data']['current_price']['usd'],  # Здесь может потребоваться другая логика для определения close
                'price_change_percentage': data['market_data']['price_change_percentage_24h_in_currency']['usd'],
                'volume': data['market_data']['total_volume']['usd'],
                'tradecount': "Не доступно"  # Подставьте реальное значение, если оно доступно через API
            }
